...
function GoogleCallbackContent() {
  ...
  useEffect(() => {
    ...
    if (error) {
      setStatus('error');
      setErrorMsg(`Google authorization failed: ${error}`);
      return;
    }

    if (!code) {
      setStatus('error');
      setErrorMsg('No authorization code provided by Google.');
      return;
    }
    ...
    const exchangeCode = async () => {
      try {
        ...
        // Refresh the user context so the app knows Google is connected
        await refreshUser();
    ...
    <div className="max-w-md w-full bg-chat-input rounded-2xl border border-chat-border p-8 text-center space-y-6">
      ...
      {status === 'loading' && (
        <>
          ...
          <h1 className="text-2xl font-bold text-foreground">Connecting Google Drive...</h1>
          <p className="text-chat-muted">Please wait while we securely connect your Google account.</p>
        </>
      )}
      ...
      {status === 'success' && (
        <>
          ...
          <p className="text-chat-muted">
            Your MemoryLLM is now securely connected to Google Drive for <strong>{accountEmail}</strong>.
          </p>
